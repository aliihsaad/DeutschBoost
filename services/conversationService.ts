...
const defaultConversationAiProvider = createGeminiAiProvider({
...
export const generateConversationFeedback = async (
  ...
  aiProvider: AiProvider = defaultConversationAiProvider
...
Be constructive, specific, and encouraging. Focus on practical improvements.`;
    ...
    const feedbackData = await generateJsonWithProvider<ConversationFeedback>(
      aiProvider,
      {
...
export const getLastConversationFeedback = async (
  ...
  try {
    ...
    let feedbackData: ConversationFeedback | null;
    if (typeof data.feedback === 'string') {
      feedbackData = safeJsonParse<ConversationFeedback>(data.feedback);
      if (!feedbackData) {
        console.error('Failed to parse feedback JSON string');
        return { feedback: null, error: new Error('Invalid feedback format') };
      }
    } else {
      feedbackData = data.feedback as ConversationFeedback;
    }
